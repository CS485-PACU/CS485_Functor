...
{
  bool operator()(std::string cLeft, std::string cRight)
  {
    bool bResult = true;
    ...
    {
      for (int i = 0; bResult && i < cLeft.length() ; ++ i)
      {
        bResult = std::toupper(cLeft.at(i)) == std::toupper(cRight.at(i));
      }
    }
    return bResult; 
  };
...
int main () {
  ...
  struct CaseInsensitiveCmp sCmp;
